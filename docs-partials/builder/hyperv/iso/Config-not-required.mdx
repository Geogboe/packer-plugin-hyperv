...
- `use_fixed_vhd_format` (bool) - If true, creates the boot disk on the
  ...
  (`.vhd`) while Generation 2 builds emit a fixed VHDX (`.vhdx`). The default
  is false, which creates a dynamic VHDX format disk. This option requires
  skip_compaction to be true and differencing_disk to be false. Additionally,
  any value entered for disk_block_size will be ignored. The most likely use
  case for this option is outputing a disk that is in the format required for
  upload to Azure.
